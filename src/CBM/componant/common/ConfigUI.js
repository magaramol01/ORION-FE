...
import sortUpLogo from '../../Images/downloadedImages/sort-up.png';
import sortDownLogo from '../../Images/downloadedImages/sort-down.png';
import addLogo from '../../Images/downloadedImages/add.png';
        ...
        <Container>
            <Row ld={8} md={8} xs={16} sm={16}>
                <div style={{
                    ...
                    <Button className="SM-button" onClick={onAddNewConfigClick}
                            variant="outline-secondary">
                        <img
                            alt=""
                            width={16}
                ...
                <Table style={{maxWidth: 800, margin: "0px auto"}} striped bordered hover size="sm">
                    <thead>
                    <tr>
                    ...
                    <tbody>
                    {
                        configList.map((configItem, index) => {
                            ...
                            return (
                                <tr
                                    key={`ctable-${index}`}>
                                    <td>{fa_alarm_text[configItem.fa_alarm_radio]}</td>
                                    <td>
                ...
                if (causeArrList.length === 1) {
                    if (causeList.length === 1) {
                        ...
                        if (index === 0) {
                            ...
                            // causeCondition += causeList[index].condition.value + causeList[index].cause.value + ")";
                            causeCondition.push([
                                (<div className={"causeCondition"} key={`${index}-${causeList[index].condition.value}`}>{causeList[index].condition.value}</div>),
                                (<div key={`${index}-${causeList[index].cause.value}`}>
                                    <CustomTooltip
                                        description={causeList[index].cause.description}
                                        label={causeList[index].cause.label}
                    ...
                    if (causeList.length === 1) {
                        ...
                        // causeCondition += causeBlockCondtion + "(" + causeList[index].cause.value + ")";
                        causeCondition.push([
                            (<div className={"causeCondition"} key={`${index}-${causeBlockCondtion}`}>{causeBlockCondtion}</div>),
                            (<div className={"causeCondition"} key={`${index}-${"("}`}>(</div>),
                            (<div key={`${index}-${causeList[index].cause.value}`}>
                                <CustomTooltip
                                    description={causeList[index].cause.description}
