...
import {addVesselDetailsToSession, getXpressMainEngineScreenData} from "../../api";
import HederUI from "../common/HederUI";

class XpressME extends Component {
    ...
    fetchMainEngineScreenState() {
        getXpressMainEngineScreenData(getVesselId(), this.onFetchDataSuccess, function () {});
    }

    onFetchDataSuccess(response) {
        if (response && response.data) {
            const mainEngineStateJson = response.data.xpressStateJson;
            ...
            if (shipNameData) {
                for (let i = 0; i < shipNameData.length; i++) {
                    if (shipNameData[i].value === "xpress-kanchenjunga") {  // for demo we have to keep china express as default in dropdown
                        defaultIndex = i;
                    }
                }
            }
    ...
    render() {
        ...
        return (
            <SMSidebar history={this.props.history} screenPath={"/XpressME"}>
...

                                            {
                                                this.state.mainEngineState ?
                                                    <DashboardGridLayout
                                                        ...
                                                        socketSubscriberName={"subscribeToXpressMainEngine"}
