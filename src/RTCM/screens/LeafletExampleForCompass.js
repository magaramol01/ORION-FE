...
class LeafletExampleForCompass extends React.Component {
        ...
        if (socketData && socketData.vesselId && this.state.vesselId === socketData.vesselId) {
            ...
            if (newStormGlassWeatherData && Object.keys(newStormGlassWeatherData).length !== 0 && newStormGlassWeatherData.constructor === Object) {
                ...
                if (!isSameEntryExists) {
                    ...
                    const vesselNewLocationStormGlassWeatherDataNmeaData = newStormGlassWeatherData.nmeaData;
                    let vesselNewLocationLatDirection = null;
                    let vesselNewLocationLongDirection = null;
                    if (vesselNewLocationStormGlassWeatherDataNmeaData && Object.keys(vesselNewLocationStormGlassWeatherDataNmeaData).length !== 0) {
                    ...
                    if (this.isValidLat(newStormGlassWeatherData.lat) && this.isValidLong(newStormGlassWeatherData.long)) {
                        leafletOptions.lat = this.getCorrectedDataPoint(newStormGlassWeatherData.lat, vesselNewLocationLatDirection);
                        leafletOptions.lon = this.getCorrectedDataPoint(newStormGlassWeatherData.long, vesselNewLocationLongDirection);
                    }
    ...
    onFetchWindyMapDataSuccess = (response) => {
        if (response && Object.keys(response.data).length !== 0) {
            ...
            if (savedStormGlassWeatherData && Object.keys(savedStormGlassWeatherData).length !== 0 && savedStormGlassWeatherData.constructor === Object) {
                leafletDataCopy.stormGlassData = savedStormGlassWeatherData;

                const savedStormGlassWeatherDataLength = savedStormGlassWeatherData.data.length;
                for (let i = savedStormGlassWeatherDataLength - 1; i >= 0; i--) {
                    const vesselLastLocationStormGlassWeatherData = savedStormGlassWeatherData.data[i];

                    const vesselLastLocationStormGlassWeatherDataNmeaData = vesselLastLocationStormGlassWeatherData.nmeaData;
                    let vesselLastLocationLatDirection = null;
                    let vesselLastLocationLongDirection = null;
                    if (vesselLastLocationStormGlassWeatherDataNmeaData && Object.keys(vesselLastLocationStormGlassWeatherDataNmeaData).length !== 0) {
                    ...
                    if (this.isValidLat(vesselLastLocationStormGlassWeatherData.lat) && this.isValidLong(vesselLastLocationStormGlassWeatherData.long)) {
                        leafletOptions.lat = this.getCorrectedDataPoint(vesselLastLocationStormGlassWeatherData.lat, vesselLastLocationLatDirection);
                        leafletOptions.lon = this.getCorrectedDataPoint(vesselLastLocationStormGlassWeatherData.long, vesselLastLocationLongDirection);
                        break;
                    }
                }
            }
            ...
            if (savedMarineTrafficData && Object.keys(savedMarineTrafficData).length !== 0 && savedMarineTrafficData.constructor === Object) {
        ...
        window.windyInit(this.state.options, windyAPI => {
            ...
            thisRef.renderPortToPortPath = function () {
                ...
                if (mapSettings.portToPortPath && Object.keys(thisRef.state.leafletData).length !== 0 && thisRef.state.leafletData.constructor === Object
                    && !Array.isArray(thisRef.state.leafletData.marineTrafficData.portToPortData)) {
                    let portToPortPath = thisRef.state.leafletData.marineTrafficData.portToPortData.ROUTE;
            ...
            thisRef.renderTravelledPath = function () {
                ...
                if (mapSettings.traveledPath && Object.keys(thisRef.state.leafletData).length !== 0 && thisRef.state.leafletData.constructor === Object) {
                    const traveledPathPointsDataArr = thisRef.state.leafletData.stormGlassData.data;
                    for (let i = 0; i < traveledPathPointsDataArr.length; i++) {
                        ...
                        if (i !== 0 && traveledPathLatLongArr.length !== 0) {
                            ...
                            const diff = getDiffBetweenDatesInMinutes(prevDataPointInfo.packetTs, dataPointInfo.packetTs);
                            if (false) {  // if data is not received for 1 hour or more
                                traveledPathLatLongArr.push([[vesselCurrentLocationLat, vesselCurrentLocationLong]]);
                            } else {
                                traveledPathLatLongArr[traveledPathLatLongArr.length - 1].push([vesselCurrentLocationLat, vesselCurrentLocationLong]);
                            }
                        } else {
                            traveledPathLatLongArr.push([[vesselCurrentLocationLat, vesselCurrentLocationLong]]);
                        }
                        ...
                        if (i === traveledPathPointsDataArr.length - 1) {
                            ...
                            let currentLocationPointMarker = L.marker([vesselCurrentLocationLat, vesselCurrentLocationLong], { icon: redDotIcon });
                            const currentLocationPointMarkerHtml = getTooltip(dataPointInfo);
                            currentLocationPointMarker.bindTooltip(currentLocationPointMarkerHtml);
                            travelledPathMarkersLayerGroup.addLayer(currentLocationPointMarker);
                        }
                    }
                    traveledPathPolyline = L.polyline(traveledPathLatLongArr, { color: '#A52A2A', weight: 1 });
            ...
            map.setView(L.latLng(thisRef.state.options.lat, thisRef.state.options.lon), thisRef.state.options.zoom);
    ...
    render() {
        ...
        return (
            <div>
                <div className="windyCompass" style={{ height: 335, width: "100%" }} id="windy" />
                <Dropdown
                ...
                >
                    ...
                    {/* voyage banner code  */}
                    {MRVLatestDatas ?
                        <div id="noonReport" style={{ width: "80%" }}>
                            <div className="row mrvDataHederRow">
                                <div className="col-md-2 col-lg-2"><p className="ml24">Vsl.Name</p></div>
                                <div className="col-md-1 col-lg-1"><p className="ml8">Voy.No.</p></div>
                                <div className="col-md-2 col-lg-2"><p className="vslNameSpace ml6" >Source Port</p></div>
                                <div className="col-md-2 col-lg-2"><p className="vslNameSpace ml6" >Dest.Port</p></div>
                                <div className="col-md-1 col-lg-2"><p className="vslNameSpace ml8" >ETA</p></div>
                                <div className="col-md-1 col-lg-1"><p className="m8">Total NM</p></div>
                                <div className="col-md-2 col-lg-2"><p className="vslNameSpace ml8" >Vsl.TZone</p></div>
                            </div>
                            <div className="row mrvDataRow">
                                <div className="col-md-2 col-lg-2"><p className="vslNameSpace ml24 lh8">{MRVLatestDatas.vessel}</p></div>
                                <div className="col-md-1 col-lg-1"><p className="ml8 lh8">{MRVLatestDatas.voyage}</p></div>
                                <div className="col-md-2 col-lg-2"><p className="vslNameSpace ml6 lh8" >{MRVLatestDatas.scr}</p></div>
                                <div className="col-md-2 col-lg-2"><p className="vslNameSpace ml6 lh8" >{MRVLatestDatas.destination}</p></div>
                                <div className="col-md-1 col-lg-2"><p className="vslNameSpace ml8 lh8" >{MRVLatestDatas.ETA}</p></div>
                                <div className="col-md-1 col-lg-1"><p className="ml8 lh8">{MRVLatestDatas.totDist} NM</p></div>
                                <div className="col-md-2 col-lg-2" ><p className="vslNameSpace ml8 lh8">{MRVLatestDatas.timezone}</p></div>
                            </div>
...
export default LeafletExampleForCompass;
