...
import {addVesselDetailsToSession, getBataviaMainEngineScreenData} from "../../api";
import HederUI from "../common/HederUI";

class BataviaME extends Component {
    ...
    fetchMainEngineScreenState() {
        getBataviaMainEngineScreenData(getVesselId(), this.onFetchDataSuccess, function () {});
    }

    onFetchDataSuccess(response) {
        if (response && response.data) {
            const mainEngineStateJson = response.data.bataviaStateJson;
            ...
            if (shipNameData) {
                for (let i = 0; i < shipNameData.length; i++) {
                    if (shipNameData[i].value === "nova-batavia-exp") {  // for demo we have to keep china express as default in dropdown
                        defaultIndex = i;
                    }
                }
            }
    ...
    render() {
        ...
        return (
            <SMSidebar history={this.props.history} screenPath={"/MainEngineYZJ"}>
...

                                            {
                                                this.state.mainEngineState ?
                                                    <DashboardGridLayout
                                                        ...
                                                        socketSubscriberName={"subscribeToBataviaMainEngine"}
