...
import {addVesselDetailsToSession, getNanjingMainEngineScreenData} from "../../api";
import HederUI from "../common/HederUI";

class NanjingME extends Component {
    ...
    constructor(props) {
        ...
        this.fetchNanjingMainEngineScreenState = this.fetchNanjingMainEngineScreenState.bind(this);
        this.onFetchDataSuccess = this.onFetchDataSuccess.bind(this);
    }

    componentDidMount() {
        document.addEventListener("keydown", escFunction, false);
        this.fetchNanjingMainEngineScreenState();
    }
    ...
    fetchNanjingMainEngineScreenState() {
        getNanjingMainEngineScreenData(getVesselId(), this.onFetchDataSuccess, function () {});
    }

    onFetchDataSuccess(response) {
        if (response && response.data) {
            const nanjingStateJson = response.data.nanjingStateJson;
            ...
            if (shipNameData) {
                for (let i = 0; i < shipNameData.length; i++) {
                    if (shipNameData[i].value === "nova-nanjing-express") {  // for demo we have to keep china express as default in dropdown
                        defaultIndex = i;
                    }
                }
            }
            ...
            this.setState({
                mainEngineState: nanjingStateJson,
                otherData: otherInfo,
            });
    ...
    render() {
        ...
        return (
            <SMSidebar history={this.props.history} screenPath={"/NanjingME"}>
...

                                            {
                                                this.state.mainEngineState ?
                                                    <DashboardGridLayout
                                                        ...
                                                        socketSubscriberName={"subscribeToNanjingME"}
